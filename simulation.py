...
class blockchain:
    def __init__(self):
        self.index = 0
        self.maxTarget = 2**args.maxTargetPower # bitcoin is 224
        self.target = (self.maxTarget**2) / (args.plyrNum * (2**256)) # expected to guess 10 times, since initial plyr num = args.plyrNum, then avg conf time = avg time length for one match
...
def main():
    ...
    with open(f'{int(time.time())}_confTime{args.confTime}_index{args.loopTime}_targetPower{args.maxTargetPower}.csv', 'a+') as f:
